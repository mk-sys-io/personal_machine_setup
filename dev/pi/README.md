...
| `pi-setup` | `tools/pi-setup.py` | `~/.local/bin/pi-setup` | `pi-setup auth` (keys + probe offer), `pi-setup models probe`/`add`/`rm` |
...
pi-setup auth   # prompt/validate the 2 provider keys (additive, resume-safe)
                # then offers to probe providers and generate curated files
pi-setup models probe --write   # re-probe a provider's catalog on demand
