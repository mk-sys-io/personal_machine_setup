...
refuses with a recovery hint. ``--now`` is the only flag (TTY-bound — no
scheduled/headless abort in this design).
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        if not args.now:
            raise AbortError(
                "abort requires --now (TTY-bound — no scheduled/headless abort)"
            )
