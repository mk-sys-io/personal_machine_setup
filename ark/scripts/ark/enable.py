...
def _run_enable() -> None:
    ...
    netmgr.guards.audit_vpn_evidence()
    opslog.ok("no VPN artifacts detected")
    netmgr.guards.audit_active_tunnels()
    opslog.ok("no active tunnel interfaces")
    netmgr.guards.audit_package_manifest_drift()
    opslog.ok("no package drift since last baseline")
