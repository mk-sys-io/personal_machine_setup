...
def lock_allowlist():
    ...
    for f in ["{{ .Env.ARK_DATA_PATH }}/infra.txt",
              "{{ .Env.ARK_DATA_PATH }}/base.txt",
              "{{ .Env.ARK_DATA_PATH }}/session.txt"]:
        total += count_domains(f)
    if total == 0:
        raise lib.SealError(
            "All allowlist files are empty. Add domains first:\n"
            "  sudo <editor> {{ .Env.ARK_DATA_PATH }}/base.txt"
        )
    subprocess.run(["{{ .Env.ARK_DATA_PATH }}/scripts/generate-dnsmasq.sh", "locked"],
                   capture_output=True, check=True, timeout=60)
    subprocess.run(["{{ .Env.ARK_DATA_PATH }}/scripts/generate-policies.sh"],
                   capture_output=True, check=True, timeout=60)
    subprocess.run(["{{ .Env.ARK_DATA_PATH }}/scripts/generate-nftables.sh", "locked"],
                   capture_output=True, check=True, timeout=60)
    with open("{{ .Env.ARK_DATA_PATH }}/mode", "w") as f:
        f.write("locked\n")
